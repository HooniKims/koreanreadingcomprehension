...
export async function handler(event) {
  ...
  const isSocratic = payload.intent === "socratic";
  const instructions = isSocratic ? SOCRATIC_INSTRUCTIONS : BASE_INSTRUCTIONS;
  const input = buildCoachInput(payload);
  const history = isSocratic ? sanitizeHistory(payload.history) : [];
